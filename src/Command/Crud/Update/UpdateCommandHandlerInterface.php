...
class UpdateCommandHandlerInterface
{
    ...
    {
        $this->generator->setName(sprintf('Update%sCommandHandlerInterface', $this->entityName));
        $this->generator->setNamespaceName(sprintf('App\Core\%1$s\Command\Update%1$sCommand', $this->entityName));

        $parameter = new ParameterGenerator('command', sprintf('Update%sCommand', $this->entityName));
        $method = new MethodGenerator('handle');
        $method->setParameter($parameter);
        $method->setReturnType(sprintf('Update%sCommandResult', $this->entityName));

        $this->generator->addMethodFromGenerator($method);

        file_put_contents(sprintf('%2$s/Update%1$sCommandHandlerInterface.php', $this->entityName, $this->path), "<?php \n\n" . $this->generator->generate());
    }
