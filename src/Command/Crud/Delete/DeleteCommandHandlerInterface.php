...
class DeleteCommandHandlerInterface
{
    ...
    {
        $this->generator->setName(sprintf('Delete%sCommandHandlerInterface', $this->entityName));
        $this->generator->setNamespaceName(sprintf('App\Core\%1$s\Command\Delete%1$sCommand', $this->entityName));

        $parameter = new ParameterGenerator('command', sprintf('Delete%sCommand', $this->entityName));
        $method = new MethodGenerator('handle');
        $method->setParameter($parameter);
        $method->setReturnType(sprintf('Delete%sCommandResult', $this->entityName));

        $this->generator->addMethodFromGenerator($method);

        file_put_contents(sprintf('%2$s/Delete%1$sCommandHandlerInterface.php', $this->entityName, $this->path), "<?php \n\n" . $this->generator->generate());
    }
